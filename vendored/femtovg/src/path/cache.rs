        ...
        for contour in &mut self.contours {
            ...
            for i in 0..points.len() {
                ...
                if p1.flags.contains(PointFlags::CORNER) {
                    // A point tagged CORNER (every move_to / line_to vertex and
                    // every bezier endpoint) but which is actually collinear
                    // with its neighbours needs no join at all. This happens
                    // when a subpath's seam is placed mid-edge (to hide a
                    // closed shape's open/close point on a straight run) or at
                    // an arc→line tangent. Bevel/Round unconditionally force a
                    // join on every corner; at a collinear point that join is
                    // degenerate and its antialiased coverage is ill-defined,
                    // which renders as a ~1px notch on the stroked edge. Only
                    // force a Bevel/Round join where the edge genuinely turns.
                    let turns = p0.dpos.dot(p1.dpos) < 0.999_999;
                    if (dmr2 * miter_limit * miter_limit) < 1.0
                        || ((line_join == LineJoin::Bevel || line_join == LineJoin::Round) && turns)
                    {
...
mod seam_tests {
    ...
    /// A closed contour whose subpath deliberately starts partway along a
    /// straight edge — the "seam" trick vdraw uses so a rounded rectangle's
    /// open/close point falls between two collinear runs instead of on a
    /// corner. femtovg tags every `move_to` / `line_to` vertex `CORNER`, and
    /// before the fix `calculate_joins` turned *every* CORNER into a BEVEL
    /// under Round/Bevel joins — even a collinear one that needs no join. The
    /// spurious round join at that collinear seam is what rendered as the ~1px
    /// notch on the stroked top edge.
    #[test]
    fn collinear_seam_point_is_not_beveled() {
        let mut path = Path::new();
        path.move_to(50.0, 0.0); // seam: mid top edge, collinear
        ...
        cache.calculate_joins(1.0, LineJoin::Round, 10.0);

        let contour = &cache.contours[0];
        let pts = &cache.points[contour.point_range.clone()];

        let flags_at = |x: f32, y: f32| {
            pts.iter()
                .find(|p| (p.pos.x - x).abs() < 1e-3 && (p.pos.y - y).abs() < 1e-3)
                .unwrap_or_else(|| panic!("point ({x},{y}) present"))
                .flags
        };

        // The collinear seam must NOT get a join.
        assert!(
            !flags_at(50.0, 0.0).contains(PointFlags::BEVEL),
            "collinear seam must not receive a join (spurious round join = 1px notch)"
        );
        // The four genuine right-angle corners must still bevel under Round.
        for (x, y) in [(100.0, 0.0), (100.0, 50.0), (0.0, 50.0), (0.0, 0.0)] {
            assert!(
                flags_at(x, y).contains(PointFlags::BEVEL),
                "real corner ({x},{y}) must still bevel"
            );
        }
    }
}
