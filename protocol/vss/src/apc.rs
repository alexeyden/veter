...
use super::frame::{
    ...
    ESC_MARK_XON, ESC_MARK_XOFF, LF, MARKER_E2R, ST_CLOSE, TAB, TILDE, XOFF, XON,
};
...
/// envelope's end.
///
/// A whole engine snapshot: vt100 grids, scrollback and every
/// uploaded image. Deliberately generous — rejecting a legitimate
/// snapshot breaks the attach, which is worse than the memory.
...
impl State {
    ...
    fn bulkable(self) -> bool {
        matches!(
            self,
            State::Idle | State::ApcOther | State::ApcVss | State::ApcOverflow
        )
    }
}
...
mod tests {
    ...
    fn over_cap_envelope_is_dropped_and_the_stream_resyncs() {
        ...
        let mut input = envelope_e2r(&vec![b'x'; 65]);
        input.extend(envelope_e2r(b"after"));
    ...
    fn over_cap_body_full_of_escapes_still_resyncs() {
        ...
        let mut input = envelope_e2r(&hostile);
        input.extend(envelope_e2r(b"ok"));
        let out = s.feed(&input);
    ...
    fn over_cap_envelope_split_across_reads_is_dropped_once() {
        let mut input = envelope_e2r(&vec![b'z'; 300]);
        input.extend(envelope_e2r(b"tail"));
