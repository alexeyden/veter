...
| `vfm` | File browser with picture previews: ranger-style column navigation and a Dolphin-style thumbnail icon grid for the current directory, plus copy/move/delete/rename, in-app previews, and config-driven file opening (`~/.config/vfm/config.toml`; pictures open in `vplay`, markdown in `vmd`). |
