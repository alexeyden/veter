...
`veter/src/theme.rs` is the single source for every colour veter paints, and for the palette it hands to programs running inside it. A `Theme` carries what a published colourscheme actually publishes — the ANSI-16 table, the background, the foreground — and *derives* the rest (panel surfaces, dim text, the warn trio, the search highlights), with every derived field pinnable. That is what makes a user file at `<config>/themes/<name>.toml` worth having: eighteen hex values produce a complete look. The four built-ins (`tokyonight-storm`, `catppuccin-mocha`, `gruvbox-dark`, `nord`) pin only their own palette; the `veter` default pins everything, which is how a config with no `[theme]` section keeps what veter painted before the module existed — bar accent slots 2 and 3, retuned since. Accent slot 2 is the one worth care: it is what a program *one level in* gets, which under `vmux` is everything, so it is on screen all day. Keep it near slot 1 in weight and away from green, which is perceptually the brightest hue and so reads louder than a blue of the same nominal saturation.
