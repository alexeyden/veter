...
| `tools/vplay` | Interactive image and video viewer for VGE-aware terminals. Left/right arrows seek in video mode and, in image mode, cycle the opened file's sibling stills (`src/playlist.rs`, lexicographical, directory scanned once at startup); `hjkl` always pans, since the arrows are taken. The still texture is uploaded with `Retention::Manual` — an `Auto` image would be refcount-collected both by the swap's own element retarget and by the resize path's `ClearAll`. |
...
| `tools/vfm` | File browser with picture previews. ranger-style navigation (ancestor columns on the left, `h`/`l` to move up/in, per-directory cursor memory) with the current directory drawn as a Dolphin-style icon grid. Everything is VGE, including the filenames — the text layer renders below VGE, so a selection bar behind a name needs the name to be a `DrawText`. Thumbnails decode on worker threads (`src/thumbs.rs`, ffmpeg for video posters); copy/move/delete run on a file-operation worker (`src/ops.rs`). Enter opens a file with its configured program (`src/config.rs`, TOML at `~/.config/vfm/config.toml`; `src/open.rs` spawns it — in-terminal for editors/vplay, which suspends the VGE UI in `main` and restores it, or detached for GUI/xdg-open); `i` is the in-app preview. |
