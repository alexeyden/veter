...
- Build one crate: `cargo build -p veter` (or `veter-host`, `vmux`, `vcat`, `vplay`, `vdraw`, `vfm`, `vmd`, `vsend`, `vrecv`, `vsd`, `vssh`, `vproto`, `vge-protocol`, `prt-protocol`, `vft-protocol`, `ses-protocol`, `vss-protocol`, `vge-ui`, `breakout`)
- Run the GUI terminal: `cargo run -p veter`
- Install `veter` plus the tool set (`vcat`, `vplay`, `vdraw`, `vfm`, `vmd`, `vmux`, `vproto`, `vsend`, `vrecv`, `vsd`, `vssh`), the `vplace` script and the Claude Code hook to `$PREFIX/bin` (default `~/.local`) plus a desktop entry: `make install` (override `PREFIX=...` to retarget; `make uninstall` removes them). `make install-remote-<arch>` cross-compiles a musl build and scp-installs it to `$REMOTE`.
...
The two integration test directories are `tools/vproto/tests/` — `roundtrip.rs` (JSON → envelope → the host's own parser → JSON) and `placement.rs` (the `vplace` script against a real pty, replayed through the host engine) — and `tools/vmd/tests/session.rs`, which runs the `vmd` binary on a real pty, plays the terminal at it (answering the VGE probe), and replays what it wrote through the host engine.
...
| `libs/vge-render` | Shared client-side helpers for rendering images to a VGE-aware terminal, plus the shared raw-TTY / poll / probe helpers every VGE client uses (`vcat`, `vplay`, `vdraw`, `vfm`, `vmd`, `spinner`, `breakout`). |
| `libs/vge-ui` | Shared client-side widget toolkit, extracted from `vmux`: host theme (`theme` — the accent plus the eight `host.*` surface/text colours, adopted from the PRT probe), rounded chrome paths (`shape`), the readline-style `LineEditor` (`edit`), the filterable `Picker` (`picker`), the prompt/picker/scrolling modal builders (`modal`), and the key + SGR-mouse `InputParser` (`input`). Pure `vge-protocol` consumer — builds draw commands and parses input, owns no state and does no I/O. Used by `vmux`, `vfm` and `vmd`. |
...
| `tools/vfm` | File browser with picture previews. ranger-style navigation (ancestor columns on the left, `h`/`l` to move up/in, per-directory cursor memory) with the current directory drawn as a Dolphin-style icon grid or, on `Tab`, as a detail list (`layout::View`; a list is the same geometry one tile wide, so scrolling and hit-testing have one implementation, and `+`/`-` sizes tiles in the grid and row heights in the list). Everything is VGE, including the filenames — the text layer renders below VGE, so a selection bar behind a name needs the name to be a `DrawText`. Thumbnails decode on worker threads (`src/thumbs.rs`, ffmpeg for video posters); copy/move/delete run on a file-operation worker (`src/ops.rs`). Enter opens a file with its configured program (`src/config.rs`, TOML at `~/.config/vfm/config.toml`; `src/open.rs` spawns it — in-terminal for editors/vplay/vmd, which suspends the VGE UI in `main` and restores it, or detached for GUI/xdg-open); `i` is the in-app preview. Resolution is user-ext → user-media → **built-in ext** (markdown → `vmd`) → built-in media (pictures → `vplay`, text → `$EDITOR`) → user default → `xdg-open`: both user levels lead both built-in ones, so a built-in never silently outranks a configured rule. Two clipboards that never mix: `y`/`d`/`p` are vfm's own, while `Y` (paths, via OSC 52) and `Ctrl+Y` (the files, as a `text/uri-list` selection) drive the system one (`src/clip.rs`). Since an X11/Wayland selection dies with the process that owns it, `Ctrl+Y` re-execs the binary as a detached `--clipboard-serve` helper that holds it; the `system-clipboard` feature (default on, off for the musl dist build) gates that half. |
| `tools/vmd` | Markdown viewer. `pulldown-cmark` parses, `src/doc.rs` rebuilds the block tree from its flat event stream, `src/layout.rs` wraps that into rows **in em units** and `src/render.rs` multiplies by the current zoom to get cells — which is why `+`/`-` is a re-layout at a different wrap width and nothing else. Headings are real `font_scale` (§7.4), code sits on an `inset_bg` plate, inline `code` gets a chip of its own, table cells wrap (columns sized by CSS's want/need rule, so the squeeze falls on the prose column), pictures are decoded on workers and uploaded `Retention::Manual` behind an LRU. One clipped element holds the page (rebuilt from the visible slice per scroll tick, so a frame costs the pane, not the file) and one holds the chrome. Hit-testing for links is local — vmd laid the runs out, so it does not need `QueryHit`. |
...
## One theme, three consumers

`veter/src/theme.rs` is the single source for every colour veter paints, and for the palette it hands to programs running inside it. A `Theme` carries what a published colourscheme actually publishes — the ANSI-16 table, the background, the foreground — and *derives* the rest (panel surfaces, dim text, the warn trio, the search highlights), with every derived field pinnable. The derivations are direction-aware — they key off whether the background is lighter than the foreground — so the two light schemes get dark text over their accents and a darkened warning label rather than lightened ones. That is what makes a user file at `<config>/themes/<name>.toml` worth having: eighteen hex values produce a complete look. The six scheme built-ins (`tokyonight-storm`/`-day`, `catppuccin-mocha`/`-latte`, `gruvbox-dark`, `nord`) pin only their own palette; the `veter` default pins everything, which is how a config with no `[theme]` section keeps what veter painted before the module existed — bar accent slots 2 and 3, retuned since. Accent slot 2 is the one worth care: it is what a program *one level in* gets, which under `vmux` is everything, so it is on screen all day. Keep it near slot 1 in weight and away from green, which is perceptually the brightest hue and so reads louder than a blue of the same nominal saturation.

It reaches three places:

- **The grid** — `renderer::Palette` holds the theme as its *base*, not as another override. `OSC 4 / 10 / 11 / 12` layer over it and `OSC 104 / 110 / 111 / 112` drop back to it, so a program that resets the palette on exit leaves the user's theme, not a hardcoded one. `PaletteBase` is flattened out of `Theme` deliberately — the render pass clones the palette once per frame, and a `Vec` in there would put an allocation on that path.
...
A session outlives renderers, so the palette a client probed can go stale: reattaching does not change any palette, it swaps in a renderer that may have a different one, and the client is still holding what it read from whichever renderer it probed first. Two halves fix that. `PrtEngine::set_host_palette` re-seeds existing portals rather than only ones created later, which repairs everything drawn as `StyleRef("host.*")` — the host re-resolves those at render time. What it cannot repair is a shade the client derived itself (`vge-ui`'s `activity_style` is `Style::Flat(darken(accent, 0.4))`, a literal baked into a command the host now stores), so PRT carries `HostThemeChanged` (§8.13) and the client recomputes and redraws. `vsd` emits it on attach — the only party that knows an attach happened, and the one piece of PRT output it sends while a renderer is attached, since the answered-exactly-once rule is about replies carrying a request id and an unsolicited event has none.

`[search]` in the config is an *override over the theme*, not a sibling of it: unset keys fall through, so naming a theme restyles the panel with it. The accents are a `[theme]` field rather than a section of their own, because several theme fields are *derived from* the accent (`text_on_accent` picks light or dark by its luminance; `search_accent` defaults to it) and those have to follow the accent actually painted — a section overwriting the accents from outside would leave near-black text on a dark accent set over a light-accented theme.
