...
**One implementation**: `veter_host::pipeline::drive_chunk`, called by `veter/src/main.rs::App::process_pty_output` for the host grid and by `prt::PrtEngine::cmd_write_portal` for every portal. It used to be written out twice, which is how the same ordering bug came to live in both.

The pipeline is **a splitting stage, then a prefix of order-insensitive byte filters, then exactly one segment-aware terminal stage**. Output from the child PTY is fed through, in order:
