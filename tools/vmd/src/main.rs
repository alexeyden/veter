...
/// Alt screen, the kitty keyboard flag, hidden cursor, clear+home,
/// button-event mouse tracking (?1002) in SGR encoding (?1006) — the
/// same set vfm/vplay/vdraw use.
///
/// `CSI > 1 u` sits *after* `?1049h` deliberately: the flag stack is
/// per screen, so a push before the swap lands on the shell's screen
/// instead of ours — leaving the flag behind us and vmd itself still
...
/// may be one that reads only legacy control bytes. vmd is usually
/// opened *by* vfm, which is exactly such a next program.
