    ...
    T: Renderer,
{
    ...
    fn fill_path_internal(&mut self, path: &Path, paint_flavor: &PaintFlavor, anti_alias: bool, fill_rule: FillRule) {
        ...
        if let CommandType::ConcaveFill { .. } = cmd.cmd_type {
            ...
            self.verts.push(Vertex::new(
                path_cache.bounds.minx - fringe_width,
                // Must be `miny - fringe_width` to match the other three
                // corners. Without it the cover quad falls short by
                // `fringe_width` at the top-left, so concave_fill's final
                // stencil-zeroing pass misses a thin sliver there. A later
                // stencil_stroke then reads that stale stencil (`EQUAL 0`
                // fails) and drops pixels — a notch on the top-left of a
